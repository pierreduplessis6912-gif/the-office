...
`NATIVE_TRANSITION_BRIEF.md`), the embers/orb redesign, the full
Office Runtime v1 build (`OFFICE_RUNTIME_V1.md`, real clock/state
machine/event bus/rooms in `app/lib/runtime/`), and — most
recently — the Word Field System (Rule 3 realized in full, the
permanent ledger removed entirely) and real-time on-device speech
recognition replacing the old record-then-upload voice pipeline. See
those documents directly, and the corresponding entries in
`DECISIONS.md`, for that work — this file's own scope has stayed
backend-focused throughout.
...
Nine patterns account for the real majority of bugs found this project, seven recurring independently, two found once but general enough to watch for:
...
9. **A prompt's own opening presumption primes the model to invent a fact rather than report its absence — the opposite failure direction from every pattern above.** Every other pattern here is about a real, stated fact being dropped, misattributed, or misrouted. This one is about a fact that was never stated at all being fabricated instead. Found live 2026-07-21: `extractScopePricing`'s system prompt opened with "a tradesperson is stating prices" — a presumption that a price exists to find, baked into the very first sentence — and on a message stating only a measurement ("twenty five square meters," no rate anywhere), the model mistook the area it had just been given back in the component list for a stated rate, fabricating a real R625 quotation from nothing. **Fix, two layers:** a real, deterministic gate that never even calls the extraction unless the transcript itself contains actual price language (regex-checked, not asked of the model) — the more important fix, since it doesn't depend on the prompt being perfect; and, secondarily, an explicit "return an empty result if nothing was genuinely stated" instruction with a real negative worked example, since a prompt that never shows what "nothing found" looks like implicitly teaches the model that finding something is always the expected outcome.
