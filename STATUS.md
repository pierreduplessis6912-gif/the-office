...
6. **Assumptions diverging from live reality — schema, infrastructure, or a third-party dependency's actual runtime behavior.** A TypeScript type change doesn't touch a live `NOT NULL` constraint (37); an `INNER JOIN` silently hides real rows with a `NULL` foreign key from a debug view (38); a PDF-parsing library type-checked and bundled cleanly, then failed at actual runtime inside the real Workers sandbox with an internal bundle-resolution error invisible to any local verification (41). **Fix:** verify against the actual live system — a direct query, `PRAGMA table_info`, or (for a dependency) a real upload against the real deployed environment — before assuming. Never reconstruct reality from memory of the code that reads it, and never assume a clean type-check or a clean install means clean runtime behavior.
