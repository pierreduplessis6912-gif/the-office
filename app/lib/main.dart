...
class _FinanceRoomContentState extends State<_FinanceRoomContent> {
  ...
  // Real, separate, deliberately narrow bulk-onboarding path, per
  // direct reasoning: a CSV has real, structured columns, no AI
  // inference needed, unlike a PDF. Does not touch the existing
  // voice-first pipeline at all - a genuinely different, new upload
  ...
  Future<void> _importCustomersCsv(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body) as Map<String, dynamic>;
        _showCsvImportSummary(context, data);
      } else {
        final data = jsonDecode(response.body) as Map<String, dynamic>?;
  ...
  void _showCsvImportSummary(BuildContext context, Map<String, dynamic> data) {
    final created = data['created'] as int? ?? 0;
    final matched = data['matched'] as int? ?? 0;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text('$created new customer${created == 1 ? '' : 's'} created', style: GoogleFonts.workSans(color: _paper, fontSize: 14)),
              const SizedBox(height: 6),
              Text('$matched already existed, matched by name', style: GoogleFonts.workSans(color: _paper, fontSize: 14)),
              if (skipped > 0) ...[
                const SizedBox(height: 6),
                Text('$skipped row${skipped == 1 ? '' : 's'} skipped - no usable name', style: GoogleFonts.workSans(color: _muted, fontSize: 14)),
              ],
              const SizedBox(height: 16),
              Align(
  ...
  void _showCsvImportError(BuildContext context, String message) {
    showDialog(
      context: context,
      builder: (context) => Dialog(
        backgroundColor: _void,
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          // Real, separate, deliberately narrow bulk-
                          // onboarding path, distinct from voice
                          // capture entirely - a CSV has real,
                          // structured columns, no AI inference
                          // needed. Placed here since customers are
                          // already conceptually relevant to Finance,
                          // given no dedicated customer-list room
                          // exists yet.
                          IconButton(
                            icon: _importingCsv
...
class _TalkArea extends StatelessWidget {
  ...
  final int ignitionSeq;
  final Color? reactColor;
  // Real bug fix, found via the real compile error: this was
  // referenced directly as _financeEmberKey (a field on
  // _OfficeHomeState) from inside this completely separate widget
